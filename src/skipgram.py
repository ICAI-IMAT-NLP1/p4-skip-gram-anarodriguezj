...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        pos_dot_prod = torch.bmm(input_vectors, output_vectors).squeeze(2)
        out_loss = torch.sum(nn.functional.logsigmoid(pos_dot_prod))

        noise_vectors = noise_vectors.transpose(1,2)

        # Compute log-sigmoid loss for incorrect classifications
        neg_dot_prod = -torch.bmm(input_vectors,noise_vectors)
        # TODO
        noise_loss = torch.sum(nn.functional.logsigmoid(neg_dot_prod))
